defmodule MyLib.LibraryTest do
  ...
  describe "books" do
    ...
    @invalid_attrs %{title: nil, author: nil, isbn: nil, publish_at: nil}
    ...
    test "create_book/1 with valid data creates a book" do
      valid_attrs = %{title: "some title", author: "some author", isbn: "some isbn", publish_at: ~N[2025-07-15 02:53:00]}

      assert {:ok, %Book{} = book} = Library.create_book(valid_attrs)
      assert book.title == "some title"
      assert book.author == "some author"
      assert book.isbn == "some isbn"
      assert book.publish_at == ~N[2025-07-15 02:53:00]
    end
    ...
    test "update_book/2 with valid data updates the book" do
      book = book_fixture()
      update_attrs = %{title: "some updated title", author: "some updated author", isbn: "some updated isbn", publish_at: ~N[2025-07-16 02:53:00]}

      assert {:ok, %Book{} = book} = Library.update_book(book, update_attrs)
      assert book.title == "some updated title"
      assert book.author == "some updated author"
      assert book.isbn == "some updated isbn"
      assert book.publish_at == ~N[2025-07-16 02:53:00]
    end
  ...
  describe "loans" do
    ...
    test "create_loan/1 with valid data creates a loan" do
      valid_attrs = %{borrowed_at: ~N[2025-07-15 02:57:00], due_at: ~N[2025-07-15 02:57:00], returned_at: ~N[2025-07-15 02:57:00]}

      assert {:ok, %Loan{} = loan} = Library.create_loan(valid_attrs)
      assert loan.borrowed_at == ~N[2025-07-15 02:57:00]
      assert loan.due_at == ~N[2025-07-15 02:57:00]
      assert loan.returned_at == ~N[2025-07-15 02:57:00]
    end
    ...
    test "update_loan/2 with valid data updates the loan" do
      loan = loan_fixture()
      update_attrs = %{borrowed_at: ~N[2025-07-16 02:57:00], due_at: ~N[2025-07-16 02:57:00], returned_at: ~N[2025-07-16 02:57:00]}

      assert {:ok, %Loan{} = loan} = Library.update_loan(loan, update_attrs)
      assert loan.borrowed_at == ~N[2025-07-16 02:57:00]
      assert loan.due_at == ~N[2025-07-16 02:57:00]
      assert loan.returned_at == ~N[2025-07-16 02:57:00]
    end
